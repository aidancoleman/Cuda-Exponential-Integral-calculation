...
//------------------------------------------------------------------------------
/*
 Alternative version of my previously committed main.cpp made by Prof. Jose Mauricio Refojo => simpler and is designed only for CPU-based exponential integral computation, whereas our previous committed version integrates both CPU and GPU execution, allowing for performance comparisons and result validation => without CUDA calls, GPU kernel launches, or result verification logic => previous version includes CUDA support.
My previously committed version uses midpoint sampling which can be more accurate for numerical integration, whereas this version samples at the edge of intervals. My other version allows for  command-line options to selectively enable/disable CPU or GPU execution (-c, -g), verbose output, and timing, giving it greater flexibility and diagnostic capability => benchmarking
 */
#include <time.h>
#include <iostream>
