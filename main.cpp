...
/* I added vectors that store GPU calculated results for later comparison against the CPU values to compare results and allow speedup measurement */
...
/* capture total runtime for each version including all setup and memory transfers  */
    double cpuStartTime, cpuEndTime, gpuStartTime, gpuEndTime;

    if (cpu) {
        struct timeval start, end;
        gettimeofday(&start, NULL);
/* We ensure ample points are shifted to midpoint of each interval + 
No zero x values unless a == b.
*/
...
for (unsigned int i = 1; i <= n; ++i) {
    ...
    if (gpu) {
        struct timeval start, end;
        gettimeofday(&start, NULL);
/* exponential integral computation task given to the GPU using CUDA. */
